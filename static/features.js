...
$(".remove_form").submit((evt) => {
  const removeMessage = confirm(
    "Warning! You are about to remove this permanently!"
  );
  removeMessage;
...
const displayBalances = (jElements) => {
  for (const item of jElements) {
    item.innerText = new Intl.NumberFormat("us-US", {
      style: "currency",
      currency: "USD",
      minimumFractionDigits: 0,
    }).format(Number(item.innerText));
...
for (const item of amounts) {
  $(item).submit((evt) => {
    ...
    $.post("/handle_entry_edit", formInputs, (res) => {
      ...
      $(".amount_form").hide();
      displayBalances(projectedBalances);
      evtAmountValue.innerText = new Intl.NumberFormat("us-US", {
        style: "currency",
        currency: "USD",
        minimumFractionDigits: 0,
      }).format(Number(item.innerText));
...
/// % ** Work-in-Progress: Adding modal ** % ///
var myModalEl = document.getElementById("return-home");
